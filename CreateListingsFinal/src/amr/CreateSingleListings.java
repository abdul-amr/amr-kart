...
public class CreateSingleListings {
	...
	public CreateSingleListings() throws Exception {
		...
		this.skuSeedsStart= configurationLoader.getLastSeedsSkuCount();
	...
	private void createFlipkartListingsForOnePhone(String mobileFolder) throws ClientProtocolException, IOException {
		...
		if("VEGETABLES".equals(mobileName) || "FRUITS".equals(mobileName) || "FLOWERS".equals(mobileName)){
			...
			if(varieties != null && varieties.length > 0){
				for(File eachType : varieties){
					...
					int tempSkuCount = this.skuSeedsStart;
					createSeedsListngs(mobileName,eachType,mobileFolder);
					int finalSkuCount = this.skuSeedsStart;
					listingsInfo += eachType.getName()+"-"+(finalSkuCount-tempSkuCount)+"\t";
				}
				configurationLoader.updateSeedsSkuCount(this.skuSeedsStart);
				listingInfoList.add("Seeds : "+mobileName+" Models Created : "+listingsInfo);
			}
		}else{
			System.out.println("Creating Listings for Mobile : "+mobileName);
			File[] varieties = parentFolder.listFiles();
	...
	private void createSeedsListngs(String mobileName, File eachType, String mobileFolder) throws ClientProtocolException, IOException {
		// TODO Auto-generated method stub
			...
			if(imageUrls != null && imageUrls.size() > 0 && caseTypes != null && caseTypes.length() > 0){
				for(String caseType : caseTypes.split(",")){
					...
					if(caseTypeArr != null && caseTypeArr.length > 0){
						List<Map<Integer, String>> completedListings = popuateEachSeedsRow(imageUrls,caseTypeArr[0],mobileName,commonImageUrls,keywords,brandColors,false,eachType);
...
//							completedListings = popuateEachSeedsRow(imageUrls,caseTypeArr[0],mobileName,commonImageUrls,keywords,brandColors,true,eachType);
...
//					System.out.print("\n");
	...
	private List<Map<Integer, String>> popuateEachSeedsRow(List<String> imageUrls, String caseType, String mobileName, List<String> commonImageUrls,
			String keywords, String[] brandColors, boolean diffBrandColors, File eachType) {
		// TODO Auto-generated method stub
		List<Map<Integer, String>> completeListings = new ArrayList<>();
		String seedsCount = (String) this.configProp.get("seeds-packet-count");
		int index = 0;
		for(String imageUrl : imageUrls){
			Map<Integer, String> eachRow = new HashMap<>();
			this.skuSeedsStart++;
			String tempSku = this.getSkuStringMap().get(eachType.getName())+""+this.skuSeedsStart;
			String[] tempSkuToken = tempSku.split("-");
			tempSku = tempSkuToken[0]+"-"+tempSkuToken[1]+"_"+seedsCount+"-"+tempSkuToken[2];
			eachRow.put(6, tempSku);
			eachRow.put(7, "Paudha");
			eachRow.put(8, this.getModelMap().get(eachType.getName())+""+this.skuSeedsStart);
			eachRow.put(9, seedsCount);
			eachRow.put(10, eachType.getName());
			eachRow.put(11, "Indoor::Outdoor");
			eachRow.put(12, "Yes");
			eachRow.put(13, "Pack of "+seedsCount+ " " + eachType.getName() +" Seeds");
			...
			String description = getDescriptionMap().get(eachType.getName());
			if(!StringUtils.isBlank(description)){
//				description = description.replaceAll("<seedname>", mobileName.toLowerCase());
				eachRow.put(20, description);
			}
			String keywords1 = keywords.replaceAll("seedname", eachType.getName().toLowerCase()).replaceAll("category", mobileName.toLowerCase());
			if(keywords1.length() >= 1000){
				keywords1 = keywords1.substring(0,999);
				keywords1 = keywords1.substring(0,keywords1.lastIndexOf(":"));
			}
			eachRow.put(21,keywords1);
			eachRow.put(22, "BEST QUALITY");
//			eachRow.put(29, "Reliable, Durable, Shock-proof, Drop-protection, Luxurious, Slim-design");
			completeListings.add(eachRow);
		}
		return completeListings;
	}
...
//										    	System.out.println(line);
	...
	private List<String> extractSeedsSpecificCatalogForOtherSellers(String sellerName, String mobiles, String types) throws FileNotFoundException, IOException {
		// TODO Auto-generated method stub
										...
										try (BufferedReader br = new BufferedReader(new FileReader(eachCatFile))) {
										    ...
										    while ((line = br.readLine()) != null) {
										    	if(sellerName.equals("MAR")){
										    		line = line.replaceAll("pau-", "pdh-");
										    	}else if(sellerName.equals("TRAM")){
										    		line = line.replaceAll("pau-", "pha-");
										    	}
//										    	System.out.println(line);
										    	listings.add(line);
										    }
...
//										    	System.out.println(line);
	...
	public void extractSeedsListingsForAllSellers(String mobileForlderNames) throws Exception {
		System.out.println("Extracting Catalog Files for All Sellers");
		List<String> amrSingleListingsCasesNCovers = extractSpecificCatalog(mobileForlderNames, "Vegetable,Fruit,Flower");
		
		List<String> tramSingleListingsCasesNCovers = extractSeedsSpecificCatalogForOtherSellers("TRAM", mobileForlderNames, "Vegetable,Fruit,Flower");

		List<String> marSingleListingsCasesNCovers = extractSeedsSpecificCatalogForOtherSellers("MAR", mobileForlderNames, "Vegetable,Fruit,Flower");
		
		//Writing Listings to File
		if(amrSingleListingsCasesNCovers.size() > 0)
			writeToFile(amrSingleListingsCasesNCovers, "AMR", "seeds",0);

		if(tramSingleListingsCasesNCovers.size() > 0)
			writeToFile(tramSingleListingsCasesNCovers, "TRAM", "seeds",0);

		if(marSingleListingsCasesNCovers.size() > 0)
			writeToFile(marSingleListingsCasesNCovers, "MAR", "seeds",0);
		
		//Create SKU vs Phone Mapping
		//AMR
		System.out.println("Loading SkuvsPhone Mappings");
		List<String> amrSkuVsPhoneMapping = new ArrayList<>();
		loadSkuVsPhoneMapping(amrSkuVsPhoneMapping, amrSingleListingsCasesNCovers,"seeds");
		writeToFileMapping(amrSkuVsPhoneMapping, "AMR");
		
		//TRAM
		List<String> tramSkuVsPhoneMapping = new ArrayList<>();
		loadSkuVsPhoneMapping(tramSkuVsPhoneMapping, tramSingleListingsCasesNCovers,"seeds");
		writeToFileMapping(tramSkuVsPhoneMapping, "TRAM");
		
		//MAR
		List<String> marSkuVsPhoneMapping = new ArrayList<>();
		loadSkuVsPhoneMapping(marSkuVsPhoneMapping, marSingleListingsCasesNCovers,"seeds");
		...
		for(String echFoilder : mobileForlderNames.split(",")){
			mobileNames.add(new File(echFoilder).getName());
//			for(File seedName : new File(echFoilder).listFiles()){
//				if(!(seedName.getName().contains("Combo") || seedName.getName().contains("Glass")  || seedName.getName().contains("Screen Guard")
//						|| seedName.getName().contains("Tempered") || seedName.getName().contains("Catalog") 
//						|| seedName.getName().contains("#"))){
//				}
//			}
		}
	...
	private static void loadSkuVsPhoneMapping(List<String> skuVsPhoneMapping,
		...
		for(String eachLine : listings){
			...
			if(caseType.equals("cases_covers")){
				...
				skuVsPhoneMapping.add(tokens[0]+"::"+tokens[5]);
			}else if(caseType.equals("seeds")){
				skuVsPhoneMapping.add(tokens[0]+"::"+tokens[8]);
			}
		}
