...
public class CreateListingsFinal {
	...
	public static void main(String[] args) throws Exception {
		...
		if(args[0].equals("SingleListing")){
			...
			//Validation
			String seedsCount = (String) createListings.configProp.get("seeds-packet-count");
			if(seedsCount == null || "".equals(seedsCount)){
				System.out.println("seeds-packet-count is missing in config.properties");
				return;
			}else if(!"10".equals(seedsCount) && !"25".equals(seedsCount) && !"50".equals(seedsCount)){
				System.out.println("seeds-packet-count can be 10 or 25 or 50");
				return;
			}
			CreateSingleListings createSingleListings = new CreateSingleListings();
			//Creating Single Listings
			createSingleListings.createFlipkartListingsForManyPhone(mobileForlderNames.split(","));
			createSingleListings.extractSeedsListingsForAllSellers(mobileForlderNames);
